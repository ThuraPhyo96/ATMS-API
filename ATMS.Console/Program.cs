...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        #region EF Core Example
        EFCoreExample eFCoreExample = new();
        BankCard bankCard = eFCoreExample.CardLogin();
        ...
        else
        {
            ...
            switch (Console.ReadLine())
            {
                case "w":
                    {
                        BankAccountDto bankAccount = eFCoreExample.UpdateBalance(bankCard.BankCardNumber, bankCard.PIN, "w");
                        if (bankAccount.StatusCode == 200)
                        {
                            Console.WriteLine($"Your available balance: " + bankAccount.AvailableBalance);
                        }
                ...
                case "d":
                    {
                        BankAccountDto bankAccount = eFCoreExample.UpdateBalance(bankCard.BankCardNumber, bankCard.PIN, "d");
                        if (bankAccount.StatusCode == 200)
                        {
                            Console.WriteLine($"Your available balance: " + bankAccount.AvailableBalance);
                        }
                ...
                case "h":
                    {
                        List<BalanceHistory> balanceHistories = eFCoreExample.GetHistoryByCard(bankCard.BankCardNumber, bankCard.PIN);
                        if (balanceHistories.Any())
                        {
